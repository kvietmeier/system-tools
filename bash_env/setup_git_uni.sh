...
# Detect and install Git natively based on OS
install_git() {
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        if command -v brew &> /dev/null; then
            ...
            echo "Homebrew not found. Please run 'xcode-select --install' manually."
            exit 1
        fi
        ...
        echo "Unsupported package manager or OS. Please install Git manually."
        exit 1
    fi
}

# Set up Git user configuration securely based on OS
configure_git() {
    ...
    # OS-Specific Credential Routing
    if [[ "$OSTYPE" == "darwin"* ]]; then
        echo -e "\nConfiguring macOS Keychain for secure credential storage..."
        git config --global credential.helper osxkeychain
    else
        echo -e "\nConfiguring local credential store for Linux..."
...
# Handle GitHub PAT separately (Works natively with both store and osxkeychain)
setup_pat() {
    local user_name="$1"
...
EOF
    ...
    echo "PAT securely injected into credential helper!"
}
