...
gcp_get_core_acct() {
    ...
    echo "gcloud: gcloud config list account --format=\"value(core.account)\""
    echo "Running as: $CoreAccount"
}
...
gcp_config_active() {
    echo "Current active gcloud configuration:"
    echo 'Running "gcloud config configurations list --filter=is_active=true --format=\"value(name)\"'
...
gcp_set_config() {
    ...
    echo "Setting active gcloud configuration to: $CONFIG_NAME"
    gcloud config configurations activate "$CONFIG_NAME" > /dev/null 2>&1 || {
        echo "Failed to activate configuration: $CONFIG_NAME"
        return 1
    }

    echo "Active configuration set to: $CONFIG_NAME"
}
...
gcp_get_orphaned_routes() {
    echo "Running gcloud compute routes list"
...
# List all subnets
gcp_list_subnets() {
    echo "Running gcloud compute networks subnets list"
    gcloud compute networks subnets list \
        --format="table(name, region, network, ipCidrRange, privateIpGoogleAccess)"
}
...
gcp_list_private_ips() {
   echo "Running "gcloud compute addresses list""
...
# Usage: gcp_manage_client_vms <start|stop|resume> [count]
# ====================================================================================
...
if command -v gcloud >/dev/null 2>&1; then
    ...
    alias gcpips=get_reserved_ips
    alias gcpcleanips=gcp_remove_ips
else
    echo "gcloud CLI not found. GCP aliases will not be available."
fi